    ...
    function SwiperContainer() {
        return {
            ...
            controller: function($scope, $element, $timeout) {
                ...
                $scope.swiper_uuid = $scope.mySwiperId || createUUID();
                // directive defaults
                var params = {
                ...
                var swiperUsdPromise = function(){
                    $timeout(function(){
                        //开启开关，可以设置F.bannerFlg为true；或者设置Swiper的mySwiperId，然后设置F.bannerFlag值为你所指定的id
                        if(F.bannerFlg===true || (!F.isEmpty(F.bannerFlg) && F.bannerFlg.indexOf($scope.swiper_uuid)!=-1)){
                            var swiper = null;
